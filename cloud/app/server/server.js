...
class _robotAgent extends Capability {
  ...
  //   this.mqtt.subscribe(
  //     '/+/+/@transitive-robotics/_robot-agent/+/metrics/#');
  //   this.mqtt.on('message', async (topic, metricsObject) => {
  //     const { device, sub } = parseMQTTTopic(topic);
  //     if (!device || !sub || sub[0] !== 'metrics') {
  //       return;
  //     }
  //     log.debug(`Received metrics for device: ${device}`, sub);
  //     let serviceName = 'robot-agent';
  //     if (sub[1] !== 'robot-agent' && sub.length > 3) {
  //       // e.g., metrics/@transitive-robotics/health-monitoring
  //       serviceName = `${sub[1]}/${sub[2]}`;
  //     }
  //     const { cpu, memory } = tryJSONParse(metricsObject.toString());
  
  //     log.debug(`Forwarding metrics for ${device} to HyperDX`, serviceName, {
  //       cpu, memory
  //     });
  //     const body = {
  //       "resourceMetrics": [
  ...
  //                   "dataPoints": [
  //                     {
  //                       "timeUnixNano": new Date().getTime() * 1e6,
  //                       "value": cpu
  //                     }
  //                   ]
  //                 },
  ...
  //     try {
  //       log.debug(`Forwarding metrics for ${device} to HyperDX`, serviceName);
  //       const response = await fetch('http://hyperdx:4318/v1/metrics', {
  //         method: 'POST',
  //         headers: {
  //           'Content-Type': 'application/json',
  ...
  //       if (!response.ok) {
  //         const errorDetails = await response.text();
  //         log.error(`Failed to send metrics to HyperDX for device: ${device}.
  //           Response status: ${response.status}, Details: ${errorDetails}`);
  //       }
  //     } catch (error) {
  //       log.error(`Failed to send metrics to HyperDX for device: ${device}`, error);
  //     }
  //   });
  // }
