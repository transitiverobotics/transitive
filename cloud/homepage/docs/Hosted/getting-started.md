...
Some of the capabilities available from Transitive Robotics rely on ROS and require a ROS master to be running. If the agent discovers that ROS is not installed, it will install a minimal set of ROS components to run a master. This master (roscore) will be started on boot of the user service if no other master is running yet at that time. If you do not want this to happen, you can set `global.noRosCore` to true in `~/.transitive/config.json`.

:::

:::tip Device IDs

The install script sets an ID for the device. This ID is generated from the sha256 sum of the id in `/etc/machine-id`, which is a unique id generated by systemd. If that file is empty, a random ID will be generated instead. If you prefer to set an ID of your own, you can do so by setting the `TR_INSTALL_HASH` environment variable before executing the `curl` command.
